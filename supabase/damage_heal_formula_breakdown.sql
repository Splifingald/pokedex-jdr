-- ============================================================================
-- Détail complet du calcul de dégâts/soin dans l'historique admin
-- ============================================================================
-- Ajoute deux champs par tour de dégâts, dans le même esprit que
-- damage_before_dice (déjà envoyé) :
--   - damage_species_xp : la composante espèce+XP seule, AVANT le
--     multiplicateur de type (jusqu'ici noyée dans damage_before_dice, qui
--     combine déjà espèce+XP × multiplicateur + dégâts de base capacité).
--   - damage_dice : le résultat du dé de dégâts, isolé (jusqu'ici
--     indissociable d'un éventuel modificateur de stat/bonus conditionnel
--     dans le total).
--
-- Le client (AutoBattleScreen) recombine ces valeurs avec ce qu'il connaît
-- déjà (dégâts de base de la capacité, bonus de type) pour afficher le détail
-- complet en admin, ex. :
--   "10 (dégâts de base) x2 (super efficace) + 4 (dégâts de la capacité)
--    + 4 (dé) = 28"
-- et pour les dégâts en % des PV restants :
--   "70% des PV restants de la cible (40) = 28"
-- Tout écart restant (modificateur de stat, dégâts additionnels
-- conditionnels...) apparaît en fin de ligne sous la forme générique
-- "+ X (additionnel)", sans détailler la cause. Le soin "Utilise ses propres
-- stats" réutilise les mêmes composantes (même formule que les dégâts, voir
-- requirement précédent).
--
-- Remplace intégralement autobattle_resolve_battle et
-- autobattle_resolve_manual_round (CREATE OR REPLACE FUNCTION exige le corps
-- complet) — aucun changement de signature, pas de DROP FUNCTION nécessaire
-- cette fois.
-- ============================================================================
