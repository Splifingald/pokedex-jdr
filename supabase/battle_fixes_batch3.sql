-- ============================================================================
-- CORRECTIFS DE COMBAT — LOT 3
-- ============================================================================
-- Remplace intégralement autobattle_resolve_battle, autobattle_start_manual_
-- battle et autobattle_resolve_manual_round (CREATE OR REPLACE FUNCTION
-- exige toujours le corps complet). Contient :
--
-- 1. Invulnérabilité, refonte complète : dure désormais jusqu'au DÉBUT du
--    prochain tour de son propriétaire (avant que sa capacité ne soit
--    résolue), et bloque TOUTE la prochaine rafale adverse s'il y en a une
--    (play_twice/play_three/repeat_until_fail/play_random) — pas juste son
--    1er coup comme avant. Introduit v_round_no, un compteur qui n'avance
--    qu'à chaque VRAI changement de tour (jamais au milieu d'une rafale).
-- 2. Modificateurs de stat / soin passif / Anti-Soin : même correctif —
--    leur durée "en tours" est désormais basée sur v_round_no, donc ne peut
--    plus expirer AU MILIEU d'une rafale (elle ne s'appliquait auparavant
--    qu'au 1er coup d'une rafale, pas aux suivants).
-- 3. Nouvelle colonne autobattle_manual_battles.round_no (persistée entre
--    les appels RPC, comme turn_no).
-- 4. Métamorph adversaire (mode Manuel) : copie désormais aussi le type et
--    la liste "super efficace" du joueur (comme en mode Auto — jusqu'ici
--    seules les capacités étaient copiées) et son sprite (opponent_image_
--    override, absent du résultat jusqu'ici).
-- 5. Sprite Métamorph retourné en miroir (scaleX(-1), côté client) pour
--    rester visuellement distinguable du vrai pokémon copié.
-- 6. NOUVEAU — Métamorph JOUEUR en mode Manuel (jusqu'ici limité à Auto) :
--    copie sprite/type/liste super efficace/dégâts de base de l'ADVERSAIRE
--    (jamais ses PV max ni son nom). Le mouvepool "connu" devient l'ensemble
--    des capacités configurées sur le niveau adverse (jusqu'à 10) — le
--    joueur choisit toujours laquelle jouer, contrairement à l'adversaire
--    Métamorph qui pioche au hasard dans le mouvepool du joueur.
-- ============================================================================
      ...
      ELSE
        ...
        ELSE
          ...
          IF v_player_heal_dot_config_amount IS NOT NULL THEN
            ...
            v_player_heal_dot_expires := v_round_no + v_player_heal_dot_config_turns;
          ...
          IF v_player_cancel_heal_duration IS NOT NULL THEN
            v_opponent_heal_disabled_expires := v_round_no + v_player_cancel_heal_duration;
      ...
      ELSE
        ...
        ELSE
          ...
          IF v_opponent_heal_dot_config_amount IS NOT NULL THEN
            ...
            v_opponent_heal_dot_expires := v_round_no + v_opponent_heal_dot_config_turns;
            v_turn_entry := v_turn_entry || jsonb_build_object('heal_dot_granted', true);
          END IF;
          IF v_opponent_cancel_heal_duration IS NOT NULL THEN
            v_player_heal_disabled_expires := v_round_no + v_opponent_cancel_heal_duration;
            v_turn_entry := v_turn_entry || jsonb_build_object('cancel_heal_applied', true);
          END IF;
...
BEGIN
  ...
  LOOP
    v_turn_no := v_turn_no + 1;
    -- Voir le même commentaire dans autobattle_resolve_battle : v_round_no
    -- n'avance que sur un VRAI nouveau tour, jamais au milieu d'une rafale.
    IF v_block_remaining IS NULL THEN
      v_round_no := v_round_no + 1;
    END IF;

    -- Voir le même commentaire dans autobattle_resolve_battle : le bouclier
    -- n'est ici qu'EXPIRÉ (au début du prochain tour du propriétaire) — le
    -- blocage lui-même est re-vérifié à chaque activation plus bas, jamais
    -- consommé en un coup, pour couvrir toute une rafale adverse.
    IF v_player_invulnerable AND v_player_invuln_granted_round IS NOT NULL AND v_round_no > v_player_invuln_granted_round + 1 THEN
      v_player_invulnerable := false;
      v_player_invuln_granted_round := NULL;
    END IF;
    ...
    IF v_attacker = 'player' THEN
      ...
      -- re-vérifié à chaque activation (bloque toute une rafale adverse).
      IF v_opponent_invulnerable AND (
        v_ability.deals_damage
        OR (v_ability.status_effect IS NOT NULL AND NOT v_player_status_reversed)
        OR v_player_stat_mod_target = 'opponent'
