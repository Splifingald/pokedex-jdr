-- ============================================================================
-- CORRECTIF : PV adversaire qui restent d'un combat Manuel abandonné
-- ============================================================================
-- Bug : en quittant un Combat Manuel EN COURS (avant victoire/défaite), la
-- ligne `autobattle_manual_battles` restait avec `outcome` NULL. En
-- redémarrant un combat sur le même niveau, le serveur la reprenait à tort
-- (PV/statuts de l'ancien combat), alors que le client, lui, ré-affichait
-- des PV pleins au premier rendu — d'où un gros saut de PV incohérent au
-- premier coup porté du nouveau combat.
--
-- Correctif : la fonction `autobattle_resolve_manual_round` gagne un nouveau
-- paramètre `p_is_new_battle boolean DEFAULT false`, que le client doit
-- passer à `true` uniquement pour le tout premier appel RPC d'un combat (là
-- où le ticket est débité). Toute ligne existante et non terminée pour ce
-- joueur/niveau est alors purgée avant de démarrer un combat neuf, exactement
-- comme le sont déjà les lignes terminées (gagnées/perdues) rejouées.
--
-- Remplace intégralement `autobattle_resolve_manual_round` (CREATE OR
-- REPLACE FUNCTION exige toujours le corps complet). Un changement
-- correspondant côté client (envoi de `p_is_new_battle`) est déjà fait dans
-- `src/components/AutoBattlePopup.tsx`.
-- ============================================================================
