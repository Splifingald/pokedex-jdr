-- ============================================================================
-- CORRECTIFS DE COMBAT — LOT 2
-- ============================================================================
-- Ce fichier REMPLACE tout ce qui touche autobattle_resolve_battle et
-- autobattle_resolve_manual_round dans les fichiers précédents
-- (sleep_confusion_order_fix.sql, manual_battle_abandon_fix.sql,
-- opponent_ability_slots_10.sql — leurs autres parties, comme les colonnes
-- opponent_ability_nom_5..10, restent valides et n'ont pas besoin d'être
-- réappliquées si c'est déjà fait). Contient :
--
-- 1. attacks.deals_damage (colonne, + CSV "Inflige dégâts") : les capacités
--    non-offensives (soin pur, buff pur, statut sur soi...) n'infligent plus
--    aucun dégât, même passif.
-- 2. Correctif "A subi des dégâts au dernier tour adverse" : reflète
--    maintenant EXACTEMENT le tour précédent de l'adversaire (plus de valeur
--    obsolète qui restait vraie après un tour de préparation/soin/raté).
-- 3. Correctif "affecté par un statut" (dégâts additionnels conditionnels) :
--    vérifie désormais le statut de L'ADVERSAIRE, pas celui de l'attaquant.
-- 4. Correctif soin "Utilise ses propres stats" : inclut maintenant les
--    dégâts du pokémon (espèce+XP+type), pas seulement ceux de la capacité.
-- 5. Invulnérabilité restreinte aux capacités qui affectent réellement
--    l'adversaire (dégâts, statut, modificateur de stat, Anti-Soin) — une
--    capacité auto-ciblée (soin sur soi, buff sur soi...) ne la consomme
--    plus pour rien. Dure toujours EXACTEMENT 1 tour adverse (consommée
--    qu'elle ait bloqué quelque chose ou non).
-- 6. Correctif durée des modificateurs de stat (% des dégâts de base,
--    "Nombre de tours") : 1 tour s'applique bien au PROCHAIN tour du
--    lanceur (pas au tour où il est posé), et s'efface après.
-- 7. Super efficace : n'est désormais possible QUE si la capacité utilisée
--    est du même type que le pokémon qui l'utilise, en plus de la table de
--    types.
-- 8. Nouveau point d'entrée autobattle_start_manual_battle : crée le combat
--    Manuel et tire au sort qui attaque en premier AVANT le choix de la 1ère
--    capacité (permet d'afficher le tirage au sort juste après le choix du
--    pokémon, comme en Combat Auto) — le ticket n'y est PAS débité (voir
--    autobattle_manual_battles.ticket_debited), il l'est toujours au 1er
--    tour réellement joué. autobattle_resolve_manual_round perd son
--    paramètre p_is_new_battle (cette responsabilité est reprise par ce
--    nouveau point d'entrée) — DROP FUNCTION nécessaire avant recréation.
-- ============================================================================

ALTER TABLE attacks ADD COLUMN IF NOT EXISTS deals_damage boolean NOT NULL DEFAULT true;

ALTER TABLE autobattle_manual_battles ADD COLUMN IF NOT EXISTS ticket_debited boolean NOT NULL DEFAULT false;
...
-- requirement). Ne débite PAS de ticket ici (voir autobattle_manual_battles.
-- ticket_debited) : reculer sans jamais jouer de tour après le tirage au
-- sort ne coûte rien, le ticket n'est débité qu'au 1er VRAI tour joué
-- (autobattle_resolve_manual_round). Toute ligne déjà existante pour ce
-- joueur/niveau (combat terminé, ou abandonné en cours) est TOUJOURS purgée
-- ici et remplacée par un combat neuf — voir le même requirement que le
-- correctif p_is_new_battle précédent, désormais porté par ce point d'entrée
-- plutôt que par le 1er tour.
...
BEGIN
  ...
  -- Rejeu idempotent : redémarrage redemandé (retry réseau) AVANT tout tour
  -- joué → même réponse, sans re-tirer first_attacker.
  IF v_row.id IS NOT NULL AND v_row.turn_no = 0 AND NOT v_row.ticket_debited
...
BEGIN
  ...
  -- qui décide first_attacker AVANT le 1er choix de capacité — voir son
  -- commentaire) : ce tour ne la crée plus lui-même.
  IF v_row.id IS NULL THEN
    RETURN jsonb_build_object('status', 'not_started');
  END IF;

  IF NOT v_row.ticket_debited THEN
    UPDATE player_items SET quantity = quantity - 1
      WHERE player_id = p_player_id AND item_nom = v_ticket_item_nom AND quantity >= 1;
    IF NOT FOUND THEN
      RETURN jsonb_build_object('status', 'no_ticket');
