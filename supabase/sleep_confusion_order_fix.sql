-- ============================================================================
-- CORRECTIF : ordre de résolution burn / poison / confusion / sommeil
-- ============================================================================
-- Ce fichier REMPLACE les fichiers envoyés précédemment
-- `confusion_rework.sql` et `sleep_fix_and_status_filter.sql`, qui
-- contenaient une logique incorrecte (le sommeil et la confusion
-- s'appliquaient TOUJOURS au moins une fois avant que le dé ne soit
-- pertinent). Ne pas exécuter ces deux anciens fichiers si ce n'est pas
-- déjà fait — exécuter uniquement celui-ci à la place pour la partie
-- sommeil/confusion (la colonne `bonus_damage_status_filter` ajoutée par
-- `sleep_fix_and_status_filter.sql` reste valide et n'a pas besoin d'être
-- réappliquée si elle l'a déjà été).
--
-- Comportement correct désormais implémenté :
--   1. Si brûlé ou empoisonné : les dégâts s'appliquent TOUJOURS (au moins
--      une fois), le dé ne détermine que si le statut persiste au tour
--      suivant.
--   2. Si brûlé, confus ou endormi : le dé est lancé et détermine
--      DIRECTEMENT l'effet de CE tour :
--        - Sommeil : le dé rate => le tour est passé ce tour-ci aussi.
--          Le dé réussit => le pokémon se réveille et agit normalement.
--        - Confusion : le dé rate => la précision est réduite de 5 ce
--          tour-ci. Le dé réussit => la confusion est levée et le
--          pokémon agit normalement, sans pénalité.
--   3. Utilisation de la capacité (si le pokémon n'est pas empêché d'agir).
--
-- Remplace intégralement les fonctions `autobattle_resolve_battle` (mode
-- Auto) et `autobattle_resolve_manual_round` (mode Manuel) — un
-- `CREATE OR REPLACE FUNCTION` doit toujours contenir le corps complet.
-- ============================================================================
