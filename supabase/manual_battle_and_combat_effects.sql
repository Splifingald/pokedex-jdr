-- ============================================================================
-- Migration autonome : effets speciaux de combat (modificateur de stat,
-- soin passif, Anti-Soin) + mode de jeu Combat Manuel.
--
-- A executer UNE FOIS dans l'editeur SQL Supabase (ou via psql), sur une
-- base ou supabase/schema.sql (l'etat courant complet du schema) n'a pas
-- encore ete rejoue avec ces ajouts. Idempotent (ADD COLUMN IF NOT EXISTS /
-- CREATE TABLE IF NOT EXISTS / CREATE OR REPLACE FUNCTION partout) : peut
-- etre relance sans risque si une partie a deja ete appliquee.
--
-- Contenu, dans l'ordre :
--   1. autobattle_variants.game_mode ('auto' / 'manual')
--   2. autobattle_levels.opponent_ability_nom_2/_3/_4 (sequence d'attaques
--      adverses en mode Manuel)
--   3. Table autobattle_manual_battles (etat vivant d'un combat Manuel)
--   4. autobattle_ability_rules : nouvelles colonnes stat_mod_*/heal_dot_*/
--      cancel_heal_duration_turns
--   5. autobattle_resolve_battle (mode Auto) mis a jour pour ces effets
--   6. autobattle_resolve_manual_round (nouvelle fonction, mode Manuel)
--
-- Depend de : autobattle_ability_burst (deja existante), autobattle_xp_bonus,
-- autobattle_max_xp (deja existantes) — non redefinies ici.
-- ============================================================================
...
-- variants) : l'adversaire peut jouer jusqu'à 4 capacités au lieu d'une
...
-- laquelle des 4 est jouée ; sans effet si l'adversaire est Métamorph, qui
...
-- seule. _2/_3/_4 restent NULL en mode Auto (ignorées) et sont optionnelles
...
BEGIN
  ...
  SELECT heal_type, heal_amount, heal_percent, turn_effect, repeat_max_iterations, status_reversed,
         ...
         heal_dot_amount, heal_dot_duration_turns, cancel_heal_duration_turns
    INTO v_player_heal_type, v_player_heal_amount, v_player_heal_percent, v_player_turn_effect, v_player_repeat_max, v_player_status_reversed,
         ...
         v_player_heal_dot_config_amount, v_player_heal_dot_config_turns, v_player_cancel_heal_duration
  ...
  SELECT heal_type, heal_amount, heal_percent, turn_effect, repeat_max_iterations, status_reversed,
    ...
    INTO v_opponent_heal_type, v_opponent_heal_amount, v_opponent_heal_percent, v_opponent_turn_effect, v_opponent_repeat_max, v_opponent_status_reversed,
         ...
         v_opponent_heal_dot_config_amount, v_opponent_heal_dot_config_turns, v_opponent_cancel_heal_duration
