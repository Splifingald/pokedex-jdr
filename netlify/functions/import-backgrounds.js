// Netlify Function — Import Fonds d'écran depuis CSV
...
exports.handler = async (event) => {
  try {
    ...
    // Supprimer tous les fonds d'écran existants
    const { error: deleteError } = await supabase.from('backgrounds').delete().neq('id', 0)
    if (deleteError) {
    ...
    for (let i = 0; i < rows.length; i += CHUNK) {
      const { error } = await supabase.from('backgrounds').insert(rows.slice(i, i + CHUNK))
      if (error) {
