// Netlify Function — Import Images du mode Affichage depuis CSV
...
exports.handler = async (event) => {
  try {
    ...
    // Supprimer toutes les images d'affichage existantes
    const { error: deleteError } = await supabase.from('display_assets').delete().neq('id', 0)
    if (deleteError) {
    ...
    for (let i = 0; i < rows.length; i += CHUNK) {
      const { error } = await supabase.from('display_assets').insert(rows.slice(i, i + CHUNK))
      if (error) {
