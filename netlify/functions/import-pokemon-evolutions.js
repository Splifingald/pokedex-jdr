// Netlify Function — Import Évolutions depuis CSV
...
exports.handler = async (event) => {
  try {
    ...
    // Supprimer toutes les évolutions existantes
    const { error: deleteError } = await supabase.from('pokemon_evolutions').delete().neq('id', 0)
    if (deleteError) {
    ...
    for (let i = 0; i < rows.length; i += CHUNK) {
      const { error } = await supabase.from('pokemon_evolutions').insert(rows.slice(i, i + CHUNK))
      if (error) {
