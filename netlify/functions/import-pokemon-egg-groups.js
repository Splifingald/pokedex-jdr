// Netlify Function — Import Groupes d'œufs (Pension Pokémon) depuis CSV
...
exports.handler = async (event) => {
  try {
    ...
    // Supprimer tous les groupes d'œufs existants
    const { error: deleteError } = await supabase.from('pokemon_egg_groups').delete().neq('id', 0)
    if (deleteError) {
    ...
    for (let i = 0; i < rows.length; i += CHUNK) {
      const { error } = await supabase.from('pokemon_egg_groups').insert(rows.slice(i, i + CHUNK))
      if (error) {
