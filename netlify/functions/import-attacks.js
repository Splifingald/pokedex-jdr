// Netlify Function — Import Attaques depuis CSV
...
exports.handler = async (event) => {
  try {
    ...
    // Supprimer toutes les attaques existantes
    const { error: deleteError } = await supabase.from('attacks').delete().neq('id', 0)
    if (deleteError) {
    ...
    for (let i = 0; i < rows.length; i += CHUNK) {
      const { error } = await supabase.from('attacks').insert(rows.slice(i, i + CHUNK))
      if (error) {
