// Netlify Function — Import Table de rencontres depuis CSV
...
exports.handler = async (event) => {
  try {
    ...
    // Supprimer toutes les rencontres existantes
    const { error: deleteError } = await supabase.from('encounters').delete().neq('id', 0)
    if (deleteError) {
    ...
    for (let i = 0; i < rows.length; i += CHUNK) {
      const { error } = await supabase.from('encounters').insert(rows.slice(i, i + CHUNK))
      if (error) {
